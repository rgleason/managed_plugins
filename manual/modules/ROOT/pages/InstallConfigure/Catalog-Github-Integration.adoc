...
possible errors if the metadata has been changed by other means. Using 
`-X theirs` means that the auto branch should be used if there are conflicts. 
...
This xmllint check is not necessary, it's done automatically on every PR when
pushed. However, it shortcuts handling of problems.

When testing is complete or just not deemed necessary: push using 
`git push origin master` or so.
